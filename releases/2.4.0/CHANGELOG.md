...
Correction de l'affichage de la date de dernière exportation.

### Corrigé

- **Date de dernière exportation affichée à l'envers.** Lors d'un export
  incrémental, la date détectée s'affichait au format US `MM/dd/yyyy`
  (ex. `12/15/2025`) selon la culture de la machine. Elle s'affiche désormais
  au format français `dd/MM/yyyy HH:mm:ss` (ex. `15/12/2025 18:05:55`), à
  l'écran comme dans le rapport d'export. Le stockage interne dans
  `last_export.txt` reste inchangé (ISO `yyyy-MM-dd HH:mm:ss`).

### Ajouté

- Fonction interne `Format-DisplayDateTime` : formatage `dd/MM/yyyy HH:mm:ss`
  forcé en culture invariante (indépendant de la culture du poste). Couverte
  par 3 tests Pester, dont un qui valide le rendu sous une culture `en-US`
  (73 tests au total).
